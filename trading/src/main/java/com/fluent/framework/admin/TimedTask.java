...
public enum TimedTask{
	...
	TWO_MIN_TO_CLOSE		( true, 	2*60*1000 	),
	ONE_MIN_TO_CLOSE		( true, 	1*60*1000 	),
	TEN_SECONDS_TO_CLOSE	( true, 	1*10*1000 	),
	...
	public final static TimedTask getTask( long nowMillis, long openMillis, long closeMillis ){
		...
		if( timeLeftMillis > CLOSING_TIME.offsetClose && timeLeftMillis < TEN_SECONDS_TO_CLOSE.offsetClose ){
			return TEN_SECONDS_TO_CLOSE;
		}
		
		if( timeLeftMillis > TEN_SECONDS_TO_CLOSE.offsetClose && timeLeftMillis < ONE_MIN_TO_CLOSE.offsetClose ){
			return ONE_MIN_TO_CLOSE;
		}
		
		if( timeLeftMillis > ONE_MIN_TO_CLOSE.offsetClose && timeLeftMillis < TWO_MIN_TO_CLOSE.offsetClose ){
			return TWO_MIN_TO_CLOSE;
		}
